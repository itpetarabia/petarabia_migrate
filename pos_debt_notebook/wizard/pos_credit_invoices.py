...
class PosCreditInvoices(models.TransientModel):
    ...
    @api.depends("partner_ids", "payment_method_id", "amount", "update_type", "new_balance")
    def _compute_totals(self):
        partners = self.partner_ids
        debts = partners._compute_partner_payment_method_debt(self.payment_method_id.id)
    ...
    @api.onchange("amount", "update_type", "partner_ids", "payment_method_id", "new_balance")
    def update_lines(self):
        p2amount = None
        debts = self.partner_ids._compute_partner_payment_method_debt(self.payment_method_id.id)
    ...
    def generate_invoices(self):
        ...
        for line in self.line_ids:
            ...
            if self.writeoff_method == "invoice":
                ...
                invoice.pay_and_reconcile(self.payment_method_id)
            else:
                vals = {
                    "partner_id": line.partner_id.id,
    ...
    def add_partners_with_debt(self):
        if self.payment_method_id:
            ...
            partner_debts = partners._compute_partner_payment_method_debt(self.payment_method_id.id)
            partners_to_add = []
            for p in partners:
