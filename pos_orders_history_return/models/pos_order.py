...
class PosOrder(models.Model):
    ...
    """ extra """
    returned_order_id = fields.Many2one('pos.order', string="Order Returned From")
    rtn_pos_reference = fields.Char(compute="_compute_rtn_pos_reference", store=True, readonly=True,
                                    string="Return Receipt Ref")

    @api.depends('returned_order_id')
    def _compute_rtn_pos_reference(self):
        for rec in self.filtered(lambda o: o.returned_order_id):
            rec.rtn_pos_reference = rec.returned_order_id.pos_reference

    @api.model
    def _order_fields(self, ui_order):
        res = super(PosOrder, self)._order_fields(ui_order)
        if ui_order.get('mode', False) in ['return', 'return_without_receipt']:
            res['returned_order'] = True
        if ui_order.get('returned_order_id', False):
            res['returned_order_id'] = ui_order.get('returned_order_id', False)
        return res
